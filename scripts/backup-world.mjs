...
const backupsDir = path.join(worldDir, 'WorldBackups')

// Ensure the WorldBackups directory exists inside the world folder
if (!fs.existsSync(backupsDir)) {
  fs.mkdirSync(backupsDir, { recursive: true })
  console.log('Created WorldBackups directory inside world folder')
}
...
const backupFileName = `${world}_backup_${timestamp}.tar.gz`
...
try {
  ...
  const tarCommand = `tar -czf "${backupFilePath}" -C "${rootDir}" "${world}"`
...
async function cleanupOldBackups() {
  try {
    ...
    const backupFiles = files.filter(file => 
      file.endsWith('.tar.gz') && file.includes('_backup_')
    )
